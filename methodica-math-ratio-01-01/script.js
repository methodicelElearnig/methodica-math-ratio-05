...
function resetScreenState2() {
  ...
  if (s2EChecked) {
    document.getElementById('s2-e-check').disabled = true;
    olySetFeedback(fbe, s2EIsAllCorrect(), s2EIsAllCorrect() ? S2_E_FEEDBACK.correct : S2_E_FEEDBACK.wrong);
  } else {
...
function s4Select(stepNum, id, btnEl) {
...
function s4RefreshPreviewRows(currentStep) {
  for (let i = 1; i <= 4; i++) {
    ...
    if (s4State.answers[i]) {
      box.textContent = S4_STEPS[i].correct;
...
/* =========================================================
   מסך 6 — מסך מעבר, data-screen="5", id="s5". שקף 23. לפי בקשה
   מפורשת בדוקס ("מסך 4 תעתיק משמה, גם התמונות/וידאו הם אותם") —
   ⚠️ עודכן (20.08.2026) — עוקב אחרי אותו עדכון כמו resetScreenState3:
   נכס-זוג-דמויות קבוע (yellow-avatar-and-turquise-avatar.mp4), לא
   עוד תלוי-בחירה, כדי לשמר את ה"אותו דבר בדיוק" המתועד בין שני
   המסכים האלה.
   ========================================================= */
function resetScreenState5() {
  setFixedCharVideo('s5-avatar', 'assets/videos/yellow-avatar-and-turquise-avatar.mp4');
}
