...
/* =========================================================
   ...
   אין נכס-וידאו ייעודי ל"מוכנים לתרגל" בנכסים, נבחרו אותן שתי פוזות
   הכי-קרובות מבחינה תוכנית שכבר שימשו בסיינים 1+2 (חשיבה/שאילה) —
   אותה מוסכמה בדיוק, טרם אושרה מול המפיקה. ראו ARCHITECTURE.md.
   ========================================================= */
...
function s1MixCheck(key) {
  ...
  if (isCorrect) {
    ...
    if (!whiteOk) whiteInput.value = cfg.correct.white;
    whiteInput.classList.toggle('correct', whiteOk); whiteInput.classList.toggle('wrong', !whiteOk);
    whiteInput.disabled = true;
    if (!darkOk) darkInput.value = cfg.correct.dark;
    darkInput.classList.toggle('correct', darkOk); darkInput.classList.toggle('wrong', !darkOk);
...
function resetScreenState4() {
  resolveCharBubbleVideo('s4-avatar', S0_AVATAR_ASSETS);
}
...
/* =========================================================
   ...
   lottery-kiosk-siblings), דרך s5SetPhoto() שקוראים לה מכל onDone.
...
VIQ_CFG_REGISTER('s5p1', {
  ...
  onDone: function () { s5SetPhoto('assets/images/girl-washing-car.jpeg', 'ילדה שוטפת מכונית'); }
});
...
SCQ_CFG_REGISTER('s5p2', {
  ...
  onDone: function () {
    s5SetPhoto('assets/images/lottery-kiosk-siblings.jpeg', 'אחים בדוכן הגרלה');
