...
/* =========================================================
   ...
   אין נכס-וידאו ייעודי ל"שאלת-השיא" בנכסים, נבחרו אותן שתי פוזות
   (חשיבה/שאילה) שכבר שימשו למסכי-מעבר קודמים בפרויקט הזה (סיינים
   3/4/5) — אותה מוסכמה בדיוק, טרם אושרה מול המפיקה.
   ========================================================= */
...
/* חלק 3 מתוך 5 — חלק-ביניים תמונתי, לא-מנוקד (בלי VIQ_CFG, בלי
   progress-question slot משלו). "המשך" בלבד: חושף סעיף ג' ומשחזר את
   הדיאגרמה הקבועה. */
function s2Part3Continue() {
  s2RestoreDiagram();
}
